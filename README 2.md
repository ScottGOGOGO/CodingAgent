...
2. Configure either the generic OpenAI-compatible variables `MODEL_API_KEY`, `MODEL_BASE_URL`, and `MODEL_NAME`, or fill one provider-specific block such as `QWEN_*`, `DEEPSEEK_*`, `OPENAI_*`, `GEMINI_*`, or `CLAUDE_*`.
...
- `QWEN_*`, `DEEPSEEK_*`, `OPENAI_*`, `GEMINI_*`, and `CLAUDE_*` are env presets only. They work when the target endpoint is OpenAI-compatible. Direct proprietary APIs that are not OpenAI-compatible still need a code adapter.
- For DeepSeek, set `MODEL_PROVIDER=deepseek`, `DEEPSEEK_API_KEY`, `DEEPSEEK_BASE_URL=https://api.deepseek.com`, and `DEEPSEEK_MODEL=deepseek-v4-flash` or `deepseek-v4-pro`. The legacy names `deepseek-chat` and `deepseek-reasoner` remain compatible for now but are scheduled to be deprecated by DeepSeek on 2026-07-24.
