...
const RUNTIME_DEPENDENCY_NAMES = new Set(["react-router-dom", "lucide-react", "framer-motion"]);
...
function getFile(files: WorkspaceFile[], filename: string): WorkspaceFile | undefined {
...
function addDependenciesToPackageJson(
  ...
  try {
    ...
    for (const [name, version] of Object.entries(dependenciesToAdd)) {
      if (RUNTIME_DEPENDENCY_NAMES.has(name)) {
...
export class ProposalValidator {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= MAX_PREFLIGHT_REPAIR_ATTEMPTS + 1; attempt += 1) {
        try {
          ...
          const currentStaticIssue = detectStaticValidationIssue(currentFiles);
          if (currentStaticIssue) {
            throw new PreflightValidationError("requirement_mismatch", "preflight static validation", currentStaticIssue);
          }
  ...
  private async tryAutofix(project: ProjectRecord, error: unknown): Promise<boolean> {
    ...
    const fixedLucideDependency = await this.ensureRuntimeDependency(project, error, "lucide-react", "^0.511.0");
    if (fixedLucideDependency) {
      return true;
    }

    const fixedFramerMotionDependency = await this.ensureRuntimeDependency(project, error, "framer-motion", "^11.11.17");
  ...
  private async ensureRuntimeDependency(project: ProjectRecord, error: unknown, packageName: string, version: string): Promise<boolean> {
    const message = error instanceof Error ? error.message.toLowerCase() : "";
